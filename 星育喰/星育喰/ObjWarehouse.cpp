...
void CObjWarehouse::Action()
{
	...
	else
	{
		...
		以下のm_key_rfをこの場所でtrueにしてしまうと、
		倉庫の位置にマウスがある時以外、どんな状況でもtrueになってしまい、
		キーフラグがキーフラグの役割を果たさなくなってしまう為、
		また、処理内容的にもこの場面でtrueにする必要性はないと判断した為、
		コメントアウトしました。
		もし、以下のm_key_rfがこの場面でtrueにしなければならない理由があるのであれば、連絡下さい。
		特に理由がなければこのコメントと共に以下のm_key_rfの処理を削除しておいて下さい。
		*/
		//m_key_rf = true;
