...
private:
	...
	bool *m_invincible_f[2];	//無敵フラグ(true:無敵　false:無敵ではない)[一定時間無敵用]

	int  *m_recast_time[2];		//ミサイルポッドのリキャストタイム[生産性効率アップ用]

	int  *m_damage[2];	//相手に与えるダメージ値[住民の士気がアップ用]
	int  *m_count[2];	//攻撃UP回数のカウント[住民の士気がアップ用]
