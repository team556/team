...
void CObjMessage::Init()
{
	...
	else if (m_Scene_id == 1)
	{
		...
		swprintf_s(m_font[3], L"｜");
		swprintf_s(m_font[4], L"よし、カーソルを合わせたな！～￥敵にカーソルを合わせるとＲ敵の情報が見れるぜ！Ｗ￥手に入る資材や敵の攻撃傾向がわかるから￥戦う前にしっかりチェックしておけよ！");
		swprintf_s(m_font[5], L"あ、そうそう。￥敵の情報にオーバーワークって書いてあるだろ？～￥あれはＲスペシャル技Ｗといって、￥要は敵が持っているＲ必殺技Ｗみたいなもんだ。");
		swprintf_s(m_font[6], L"今Ｙ%sＷはスペシャル技を持ってはいないが￥Ｒ敵を捕食すると敵が持っているスペシャル技を￥奪って使えるようになるＷからな。～￥じゃあ戦闘にいってみるか。",g_Player_Name);
		swprintf_s(m_font[7], L"｜");
	...
	else if (m_Scene_id == 2)
	{
		...
		swprintf_s(m_font[47], L"これが最後だな。￥Ｒスペシャル技Ｗってやつだ。￥名前がダサいのは気にすんな。");
	...
	else if (m_Scene_id == 8)
	{
		...
		swprintf_s(m_font[18], L"詳しい条件はＲ一番下のウインドウに載っているＷぜ。");
		...
		swprintf_s(m_font[37], L"右下のポッドウインドウは￥現在装備しているコアとポッドが確認できたり……￥ってかんじでな。");
