...
void CObjMessage::Init()
{
	...
	wchar_t font_data[FONT_DATA_Y][FONT_DATA_X] =
	{
		...
		{ L'わ',L'を',L'ん',L'ワ',L'ヲ',L'ン',L'プ',L'ッ',L'確',L'認',L'爆',L'功',L'図',L'送' },
		{ L'×',L'※',L'：',L'＋',L'－',L'…',L'。',L'！',L'？',L'、',L'＆',L'．',L'＝',L'０',L'１',L'２',L'３',L'４',L'５',L'６',L'７',L'８',L'９',L'一',L'二',L'三',L'四',L'五',L'六',L'七',L'八',L'九',L'零' },
		{ L'今',L'日',L'人',L'類',L'統',L'括',L'山',L'田',L'先',L'敵',L'惑',L'星',L'情',L'報',L'不',L'意',L'打',L'喰',L'前',L'行',L'鉄',L'木',L'／',L'達',L'互',L'生',L'存',L'合',L'弱',L'肉',L'名' },
		...
		{ L'有',L'利',L'不',L'利',L'赤',L'青',L'緑',L'灰',L'色',L'対',L'等',L'察',L'簡',L'単',L'活',L'用',L'数',L'字',L'上',L'真',L'中',L'線',L'Ａ',L'Ｂ',L'Ｃ' },
		{ L'吸',L'寄',L'逆',L'自',L'分',L'同',L'士',L'引',L'力',L'衝',L'突',L'大',L'小',L'宇',L'宙',L'常',L'識',L'覚',L'習',L'慣',L'考',L'関',L'係',L'評',L'価' },
		{ L'素',L'材',L'移',L'所',L'配',L'置',L'便',L'番',L'最',L'高',L'体',L'成',L'果',L'勝',L'昇',L'多',L'書',L'材',L'限',L'後',L'─',L'被',L'余',L'裕',L'指' },
	...
	//	swprintf_s(m_font[0], L"ＯあかあかＢきくＧきく＿かうい～かかかかかかかかきなきなにあ￥にあまむまむ￥テスト１２３￥２１３２３１２￥１３１２１３２");//メッセージ１
	...
	else if (m_Scene_id == 1)
	{
		...
		swprintf_s(m_font[3], L"｜");
		swprintf_s(m_font[4], L"よし、カーソルを合わせたな！～￥敵にカーソルを合わせるとＯ敵の情報が見れるＷぜ！￥手に入る資材や敵の攻撃傾向がわかるから￥戦う前にしっかりチェックしておけよ！");
		swprintf_s(m_font[5], L"あ、そうそう。￥敵の情報にオーバーワークって書いてあるだろ？～￥あれはＯスペシャル技Ｗといって、￥要は敵が持っているＯ必殺技Ｗみたいなもんだ。");
		swprintf_s(m_font[6], L"今Ｙ%sＷはスペシャル技を持ってはいないが￥敵を捕食するとＯ敵が持っているスペシャル技を￥奪って使えるようになるＷからな。～￥じゃあ戦闘にいってみるか。",g_Player_Name);
		swprintf_s(m_font[7], L"｜");
	...
	else if (m_Scene_id == 2)
	{
		...
		swprintf_s(m_font[4], L"両方の惑星の下に緑色のバーがあるだろ？～￥この下のバーが惑星のＯＨＰゲージＷになってるぜ。");
		swprintf_s(m_font[5], L"ＨＰゲージは惑星のサイズに直結していて、￥Ｏこのゲージがゼロになると￥戦いに負けてしまうＷからな。～￥ＨＰゲージが無くならないように気をつけろよ。");
		swprintf_s(m_font[6], L"次は戦闘の仕方についてだな。");
		swprintf_s(m_font[7], L"下に五つのボタンがあるだろ？");
		swprintf_s(m_font[8], L"この内の右の四つのボタンのどれかを押してみろ。");
		swprintf_s(m_font[9], L"｜");
		swprintf_s(m_font[10], L"ボタンを押したら何か出てきたろ？￥これがＯポッドＷだぜ。～￥ボタンを押すと１００人の住民が命令を受けて、￥ポッドに乗り込み、攻撃しに行っているんだ。");
		swprintf_s(m_font[11], L"んで、左下の数字は￥それぞれのポッドの残り住民数を表してる。～￥Ｏこの数が０になるとそのポッドが￥出撃できなくなるＷから要注意だぜ。");
		swprintf_s(m_font[12], L"あと、最初は準備しているから￥すぐにポッドを出撃することができるが、￥Ｏ二回目以降は準備に時間がかかるＷから￥そこも気をつけろよ。");
		swprintf_s(m_font[13], L"ポッドにはＯ属性があって有利不利があるＷんだ。～￥赤色は緑色に強く緑は青に強いそして青は赤に強い、￥灰色はすべての色に対して少し強いが￥ミサイルに対してはすごく弱い……ってかんじでな。");
		swprintf_s(m_font[14], L"ポッドは惑星に当たるとダメージを与えて、￥Ｏ惑星のサイズをちっさくできるＷぜ。～￥ちなみにＯ青い爆発は攻撃成功の合図Ｗだから￥覚えておくといいぜ。");
		swprintf_s(m_font[15], L"あ、そうそう。￥ポッドの上に表示されてるアイコンのことだが、￥こいつは今は気にすんな。～￥後で説明してやるからよ。");
		swprintf_s(m_font[16], L"次はＯミサイルＷについての説明だ。");
		swprintf_s(m_font[17], L"このボタンを押してみな。");
		swprintf_s(m_font[18], L"｜");
		swprintf_s(m_font[19], L"察しの通り、ミサイルが発射されるぜ。");
		swprintf_s(m_font[20], L"ミサイルは簡単に言うとジャブみたいなもんだ。～￥ポッドと同じでダメージを与えることができるが￥Ｏ攻撃力が低いＷ。");
		swprintf_s(m_font[21], L"ただその代わりに、￥Ｏポッドよりも準備する時間が早いＷから￥うまいこと活用してくれ！");
		swprintf_s(m_font[22], L"ちなみにポッドもミサイルも￥Ｏボタンの下に書いているキーを押しても￥命令できるＷぜ。");
		swprintf_s(m_font[23], L"真ん中に三つの線があるだろ？～￥この線はポッドやミサイルの￥発射線を可視化したものだぜ。～￥オレ達はこの線のことをＯレーンＷって呼んでる。");
		swprintf_s(m_font[24], L"んで、このレーンの一番上をクリックしてくれ。");
		swprintf_s(m_font[25], L"｜");
		swprintf_s(m_font[26], L"クリックしたレーンが他よりも白くなっているだろ？～￥これがＯレーン選択ができた状態Ｗだ。");
		swprintf_s(m_font[27], L"さっきのポッドやミサイルは￥Ｏ選択レーン上に射出されるＷから￥状況に応じてレーンを選んで戦っていけよ。");
		swprintf_s(m_font[28], L"ただし同じレーンで、￥ミサイルやポッドを撃ちすぎると￥Ｏポッドの防御力が下がっちまうデメリットＷがある。");
		...
		swprintf_s(m_font[36], L"時間切れになって衝突したら、￥Ｏ惑星のサイズが大きい方が小さい方を捕食するＷぜ。～￥これがこの宇宙の常識だから覚えとけよ。");
		swprintf_s(m_font[37], L"まあ習うより慣れろだな。～￥ここからは自分で考えて戦ってみな。￥Ｏ※クリックで戦闘が始まります。");
		swprintf_s(m_font[38], L"｜");
		swprintf_s(m_font[40], L"時間切れにならなくても￥Ｏ敵惑星の体力を削りきると￥惑星が吸い寄せられて捕食できるＷぜ。～￥逆にＯ削りきられると自分が食われちまうＷからな。");
		swprintf_s(m_font[41], L"｜");
		swprintf_s(m_font[44], L"よし！戦いに勝って捕食することができたな。～￥捕食するとＯ能力や素材を奪うことができるＷんだ。￥最高だろ？～￥さて今回の成果を見ていこうぜ。");
		swprintf_s(m_font[45], L"一番上がＹ%sＷに移住したＯ住民Ｗの数だな。～￥こいつらはＯ兵士や研究員にしたりと￥色んな所に配置できるＷから便利だぜ。",g_Player_Name);
		swprintf_s(m_font[46], L"二番目に書いているのは惑星を食べたことで￥どれだけＹ%sＷのＯ最大ＨＰＷが￥上昇したかを表す数字だな。",g_Player_Name);
		swprintf_s(m_font[47], L"ＯＨＰがあればあるほど￥戦いは負けにくくなるＷから、￥多いに越したことはねえな。");
	...
	if (m_Scene_id == 7)
	{
		...
		swprintf_s(m_font[0], L"戦いに勝ててよかったぜ。￥……だけど油断はするなよ。￥この宇宙は弱い奴には優しくないからな。");
		swprintf_s(m_font[1], L"これからも戦いに勝っていくためには、￥Ｏ自分を強くすることＷも大切だぜ！～￥……ってことで強くなる方法を教えてやるぜ！");
	...
	else if (m_Scene_id == 8)
	{
		...
		swprintf_s(m_font[2], L"｜");
		swprintf_s(m_font[3], L"ここはＯ研究所Ｗだ。～￥研究所を強化すると住民の技術が上がって￥Ｏポッドの耐久力が増すＷぜ。");
		swprintf_s(m_font[4], L"左下のボタンを押すと￥研究所を強化することができるぞ。～￥ただしＹ%sＷのＯＨＰと￥他の星から奪った資材が必要Ｗだけどな。",g_Player_Name);
		swprintf_s(m_font[5], L"右上のボタンで住民を研究員に割り振れるぜ。～￥割り振るとＯミサイルの再生産を早めたり￥コアを製作することができるＷぜ。");
		swprintf_s(m_font[6], L"おっと、ＯコアＷについての説明がまだだったな。");
		swprintf_s(m_font[7], L"ポッドはコアに蓄積された￥ＯコスモパワーＷって言うエネルギーを￥動力源として動いたり￥攻撃するエネルギー弾になったりしてるんだ。");
		swprintf_s(m_font[8], L"コアを強化することによって￥蓄積できるコスモパワーが増えて￥Ｏ攻撃力が上がるＷぜ。");
		...
		swprintf_s(m_font[17], L"ここではミサイルについての情報が見れるぜ。～￥ミサイルの再生産スピードは￥Ｏ研究員を増やすと早くなっていくＷぜ。～￥Ｏ研究所自体のレベルも上げないと意味はないＷけどな。");
		swprintf_s(m_font[18], L"詳しい条件は一番下のウインドウに載っているぜ。");
		swprintf_s(m_font[19], L"次はこのボタンをクリックしてくれ。");
		swprintf_s(m_font[20], L"｜");
		swprintf_s(m_font[21], L"ここではコアやポッドを作ることができるぜ。～￥今は資材が何もないから何も作ることはできないが、￥作るときにはＯ研究員と資材が必要Ｗだぜ。～￥Ｏ制作時に研究員は減らないＷから安心しな。");
		swprintf_s(m_font[22], L"コアを作成すると、￥蓄積できるコスモパワーが増えて￥Ｏ攻撃力が上がるＷぜ。");
		swprintf_s(m_font[23], L"ポッドを作成すると、￥ポッドの装甲が強化されて￥Ｏ耐久力があがるＷぜ。");
		swprintf_s(m_font[24], L"次はここをクリックしてみな。");
		swprintf_s(m_font[25], L"｜");
		swprintf_s(m_font[26], L"ここはＯ兵舎Ｗだ。～￥兵舎を強化すると質のいい訓練になって￥Ｏポッドの攻撃力を上げることができるＷぜ。");
		swprintf_s(m_font[27], L"ポッドには住民が乗り込んで攻撃するだろ？～￥住民を強化することによって￥ポッドの攻撃力も相対的に上がるってことだな。");
		swprintf_s(m_font[28], L"左下のボタンを押すと￥兵舎を強化することができるぞ。～￥ただし兵舎もＹ%sＷのＯＨＰと￥他の星から奪った資材が必要Ｗだけどな。",g_Player_Name);
		swprintf_s(m_font[29], L"右上のボタンで住民を兵士に割り振れるぜ。～￥ポッド一つにつき￥１００人乗せて飛んでいくんだ。");
		swprintf_s(m_font[30], L"もちろん住民の振り分けを￥行っていない色のポッドは￥Ｏ戦闘中に出撃することは￥できないＷからそこは要注意だぜ。");
		swprintf_s(m_font[31], L"次はここをクリックしてみな。");
		swprintf_s(m_font[32], L"｜");
		swprintf_s(m_font[33], L"ここはＯ倉庫Ｗだ。～￥色んな情報を見れるぜ。");
		...
		swprintf_s(m_font[40], L"最後に補足なんだが、￥研究所や兵舎を強化していくと￥Ｏ惑星が発展していくＷぜ！～￥だからどんどん強化してくれよ！");
	...
	else if (m_Scene_id == 15)
	{
		...
		swprintf_s(m_font[2], L"ここでＯスペシャル技の装備ができるＷぜ。～￥今はまだ一つしかスペシャル技がないが￥敵を食べることで増えていくからな。");
	...
	else if (m_Scene_id == 16)
	{
		...
		swprintf_s(m_font[1], L"発動は簡単だぜ。￥Ｏ右上のボタンを押せばいいだけＷだからな。");
		swprintf_s(m_font[2], L"ただしＯ戦闘中に一回しか￥発動することができねえＷから￥発動タイミングを見極めな。");
		swprintf_s(m_font[3], L"ちなみにスペシャル技は￥Ｏエンターキーでも発動できるＷからな。");
		swprintf_s(m_font[4], L"｜");
...
void CObjMessage::Action()
{
	...
	if (m_run_switch == true)
	{
		...
		else if (m_Scene_id == 2 && m_progress == 45)
		{
			m_black_out_f = false;//徐々に明転
		...
		else if (m_Scene_id == 2 && (m_progress == 46 || m_progress == 47))
		{
			//矢印表示
			m_arrow_y = 235.0f;
		}
		else if (m_Scene_id == 2 && m_progress == 48)
		{
			//矢印表示
			m_arrow_x = 550.0f;
			m_arrow_y = 310.0f;
		}
		else if (m_Scene_id == 2 && (m_progress == 49 || m_progress == 50))
		{
			//メッセージウインドウ位置変更
			m_is_top_or_bottom = 1;
	...
	else if (m_run_switch == false)
	{
		...
		else if (m_Scene_id == 2 && m_progress == 18)
		{
			//ミサイルが１つ出れば、この処理に入る
			if (m_arrow_display_f == 0)
			{
				...
				if (m_black_out_a >= 0.5f)
				{
					...
					m_run_switch = true;//メッセージ表示機能動作開始
					m_count = 0;//m_countを初期化
					m_progress = 19;//進行度上昇させてこの処理から抜ける
				}
				//1秒後、画面暗転処理開始
				else if (m_count >= 60 * 1)
				{
		...
		else if (m_Scene_id == 2 && m_progress == 25)
		{
			//レーン選択出来れば、この処理に入る
			if (m_arrow_display_f == 0)
			{
	...
	if (m_font[m_progress][m_font_count] == L'～')
	{
		...
		if (m_mou_l == true && m_key_f == false || m_skip_f == true)
		{
			...
			//※'Ｏ'等の文字は内部処理の命令文であり、描画する訳ではないため。
	...
	//▼メッセージカラー変更処理
	if (m_font[m_progress][m_font_count] == L'Ｏ' || m_font[m_progress][m_font_count] == L'Ｇ' ||
		m_font[m_progress][m_font_count] == L'Ｙ' || m_font[m_progress][m_font_count] == L'Ｗ')
	{
		if (m_font[m_progress][m_font_count] == L'Ｏ')
		{
			//Ｏ宣言の後ろにあるフォント全てを橙色に変更する
...
void CObjMessage::Draw()
{
	...
	float yellow[4] = { 1.0f,1.0f,0.0f,1.0f };//山田文字画像用
	...
	//▼山田文字画像表示
	FontDraw(L"山田", 255.0f, window_pos_y[m_is_top_or_bottom] + 8.0f, 36.0f, 36.0f, yellow, false);
