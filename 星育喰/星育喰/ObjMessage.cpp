...
void CObjMessage::Init()
{
	...
	else if (m_Scene_id == 2)
	{
		...
		swprintf_s(m_font[16], L"次はＲミサイルＷについての説明だ。");
		swprintf_s(m_font[17], L"このボタンを押してみな。");
		swprintf_s(m_font[18], L"｜");
		swprintf_s(m_font[19], L"察しの通り、ミサイルが発射されるぜ。");
		swprintf_s(m_font[20], L"ミサイルは簡単に言うとジャブみたいなもんだ。～￥ポッドと同じでダメージを与えることができるが￥Ｒ攻撃力が低いＷ。");
		swprintf_s(m_font[21], L"ただその代わりに、￥Ｒポッドよりも準備する時間が早いＷから￥うまいこと活用してくれ！");
		swprintf_s(m_font[22], L"ちなみにポッドもミサイルも￥Ｒボタンの下に書いているキーを押しても￥命令できるＷぜ。");
		swprintf_s(m_font[23], L"真ん中に三つの線があるだろ？～￥この線はポッドやミサイルの￥発射線を可視化したものだぜ。～￥オレ達はこの線のことをＲレーンＷって呼んでる。");
		swprintf_s(m_font[24], L"んで、このレーンの一番上をクリックしてくれ。");
		swprintf_s(m_font[25], L"｜");
		swprintf_s(m_font[26], L"クリックしたレーンが他よりも白くなっているだろ？～￥これがＲレーン選択ができた状態Ｗだ。");
		swprintf_s(m_font[27], L"さっきのポッドやミサイルは￥Ｒ選択レーン上に射出されるＷから￥状況に応じてレーンを選んで戦っていけよ。");
		swprintf_s(m_font[28], L"ただし同じレーンで、￥ミサイルやポッドを撃ちすぎると￥Ｒポッドの防御力が下がっちまうデメリットＷがある。");
		swprintf_s(m_font[29], L"ポッドやミサイルを撃つ度に￥レーンの色や￥ポッドの上のアイコンが赤くなっていくんだが、￥それはこのデメリット度合いを表してるってわけだ。");
		swprintf_s(m_font[30], L"要はＲ赤くなればなるほど￥ポッドの防御力が下がってヤバいＷってことだな。");
		swprintf_s(m_font[31], L"このデメリットは￥他のレーンにポッドやミサイルを出撃させていけば￥次第にＲそのレーンに出撃する￥ポッドの防御力がもとに戻るＷぜ。");
		swprintf_s(m_font[32], L"要はＲ同じレーンに撃ちつづけるのは￥やめた方がいいＷってことだぜ。");
		swprintf_s(m_font[33], L"あ、ちなみにレーン選択は￥Ｒ方向キーの上下でも操作できるＷからな。");
		swprintf_s(m_font[34], L"さて、これが戦闘最後の説明だ。");
		swprintf_s(m_font[35], L"画面上に時間が表示されてるだろ？～￥この時間は惑星同士が引力で引き合って￥衝突するまでの大体の時間だぜ。");
		swprintf_s(m_font[36], L"時間切れになって衝突したら、￥Ｒ惑星のサイズが大きい方が小さい方を捕食するＷぜ。～￥これがこの宇宙の常識だから覚えとけよ。");
		swprintf_s(m_font[37], L"まあ習うより慣れろだな。～￥ここからは自分で考えて戦ってみな。￥Ｒ※クリックで戦闘が始まります。");
		swprintf_s(m_font[38], L"｜");
		swprintf_s(m_font[40], L"時間切れにならなくても￥Ｒ敵惑星の体力を削りきると￥惑星が吸い寄せられて捕食できるＷぜ。～￥逆にＲ削りきられると自分が食われちまうＷからな。");
		swprintf_s(m_font[41], L"｜");
		swprintf_s(m_font[44], L"よし！戦いに勝って捕食することができたな。～￥捕食するとＲ能力や素材を奪うことができるＷんだ。￥最高だろ？～￥さて今回の成果を見ていこうぜ。");
		swprintf_s(m_font[45], L"一番上がＹ%sＷに移住したＲ住民Ｗの数だな。～￥こいつらはＲ兵士や研究員にしたりと￥色んな所に配置できるＷから便利だぜ。",g_Player_Name);
		swprintf_s(m_font[46], L"二番目に書いているのは惑星を食べたことで￥どれだけＹ%sＷのＲ最大ＨＰＷが￥上昇したかを表す数字だな。",g_Player_Name);
		swprintf_s(m_font[47], L"ＲＨＰがあればあるほど￥戦いは負けにくくなるＷから、￥多いに越したことはねえな。");
		swprintf_s(m_font[48], L"その下は獲得したＲ資材Ｗのことを書いてあるぜ。～￥まあ今回は資材を獲得できなかったみたいだけどな。～￥次の戦いからはＲ集められる資材Ｗのことも考えろよ。￥資材は有限だからな。");
		swprintf_s(m_font[49], L"これが最後だな。￥さっきも言ったＲスペシャル技Ｗってやつだ。");
		swprintf_s(m_font[50], L"これはＲ戦闘においての切り札Ｗみたいなもんで￥ピンチを乗り切ったりすることができるぜ。～￥またあとで詳しく説明してやるよ。");
		swprintf_s(m_font[51], L"説明はこんなもんだな。～￥Ｒ自分が何が欲しいかをしっかり把握して￥どの敵と戦うかを考えていくＷのが￥これから生き残っていく秘訣だぜ。");
