...
void CObjRocket::Action()
{
	...
	if (battle_end == false)
	{
		...
		if (hit->CheckElementHit(ELEMENT_PLAYER) == true && m_type != 0 && m_fight == false)	//惑星に当たった時かつ敵弾
		{
			//惑星と接触しているかどうかを調べる
			m_del = true;
		...
		else if (hit->CheckElementHit(ELEMENT_ENEMY) == true && m_type == 0 && m_fight == false)//敵の惑星に当たった時かつ自弾
		{
			//惑星と接触しているかどうかを調べる
			m_del = true;
			hit->SetInvincibility(true);
