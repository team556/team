...
void CObjRocket::Init()
{
	...
	else if (m_type == 1)//たこ焼き
	{		
		m_pod_max_hp = 10.0f;
	}
	else if (m_type == 2)//モアイ
	{
		m_pod_max_hp = 15.0f;
	}
	...
	else if (m_type == 5)//アマテラス
	{
		m_pod_max_hp = 20.0f;
	}
	else if (m_type == 6)//チュートリアル
	{
		m_pod_max_hp = 10.0f;
	}
...
void CObjRocket::Action()
{
	...
	if (battle_end == false)
	{
			...
			hit->CheckElementHit(ELEMENT_RKT) == true)&& m_type != 0)
		{
			...
			if (ButtonUE == 1)		//敵の種類１(レッド)がプレイヤーのポッドと当たった場合
			{
				if (m_atk_cnt == 0)	//0の時のみ、各ダメージを受ける
				{
					if (hit->CheckObjNameHit(OBJ_PODS) != nullptr)			//プレイヤーのグリーンポッド当たり時のHP
					{
						...
						m_podhp -= m_Enemy_damage * damage_buff[0] * 0.4f * m_One_pat_dem + (float)g_Spe_equip_Level + (float)(g_Bar_Level - 1);
					}
					else if (hit->CheckObjNameHit(OBJ_PODD) != nullptr)		//プレイヤーのブルーポッド当たり時のHP
					{
						...
						m_podhp -= m_Enemy_damage * damage_buff[0] * 1.5f * m_One_pat_dem + (float)g_Def_equip_Level + (float)(g_Bar_Level - 1);
					}
			...
			else if (ButtonUE == 2)	//敵の種類２(ブルー)がプレイヤーのポッドと当たった場合
			{
				if (m_atk_cnt == 0)	//0の時のみ、各ダメージを受ける
				{
					if (hit->CheckObjNameHit(OBJ_PODP) != nullptr)			//プレイヤーのレッドポッド当たり時のHP
					{
						//m_Enemy_damage += (float)g_Pow_equip_Level;//プレイヤーの火力を装備レベルによって変える(+ポッドレベル)
						//m_Enemy_damage += (float)(g_Bar_Level - 1);//決まった火力+兵舎Lv
						...
						m_podhp -= m_Enemy_damage * damage_buff[0] * 0.4f * m_One_pat_dem + (float)g_Pow_equip_Level + (float)(g_Bar_Level - 1);
					}
					else if (hit->CheckObjNameHit(OBJ_PODS) != nullptr)		//プレイヤーのグリーンポッド当たり時のHP
					{
						...
						m_podhp -= m_Enemy_damage * damage_buff[0] * 1.5f * m_One_pat_dem + (float)g_Spe_equip_Level + (float)(g_Bar_Level - 1);
					}
					else if (hit->CheckObjNameHit(OBJ_PODD) != nullptr)		//プレイヤーのブルーポッド当たり時のHP
					{
						//m_Enemy_damage += (float)g_Def_equip_Level;//プレイヤーの火力を装備レベルによって変える(+ポッドレベル)
			...
			else if (ButtonUE == 3)	//敵の種類３(グリーン)がプレイヤーのポッドと当たった場合
			{
				if (m_atk_cnt == 0)	//0の時のみ、各ダメージを受ける
				{
					if (hit->CheckObjNameHit(OBJ_PODP) != nullptr)			//プレイヤーのレッドポッド当たり時のHP
					{
						...
						m_podhp -= m_Enemy_damage * damage_buff[0] * 1.5f * m_One_pat_dem + (float)g_Pow_equip_Level + (float)(g_Bar_Level - 1);
					}
					else if (hit->CheckObjNameHit(OBJ_PODD) != nullptr)		//プレイヤーのブルーポッド当たり時のHP
					{
						//m_Enemy_damage += (float)g_Def_equip_Level;//プレイヤーの火力を装備レベルによって変える(+ポッドレベル)
						...
						m_podhp -= m_Enemy_damage * damage_buff[0] * 0.4f * m_One_pat_dem + (float)g_Def_equip_Level + (float)(g_Bar_Level - 1);
					}
					else if (hit->CheckObjNameHit(OBJ_PODS) != nullptr)		//プレイヤーのグリーンポッド当たり時のHP
					{
						//m_Enemy_damage += (float)g_Spe_equip_Level;//プレイヤーの火力を装備レベルによって変える(+ポッドレベル)
			...
			else if (ButtonUE == 4)	//敵の種類４(バランス)がプレイヤーのポッドとミサイルに当たった場合
			{
				if (m_atk_cnt == 0)	//0の時のみ、ダメージを受ける
				{
					if (hit->CheckObjNameHit(OBJ_PODP) != nullptr)			//プレイヤーのレッドポッド当たり時のHP
					{
						...
						m_podhp -= m_Enemy_damage * damage_buff[0] * 1.25f * m_One_pat_dem + (float)g_Pow_equip_Level + (float)(g_Bar_Level - 1);
					}
					else if (hit->CheckObjNameHit(OBJ_PODD) != nullptr)		//プレイヤーのブルーポッド当たり時のHP
					{
						//m_Enemy_damage += (float)g_Def_equip_Level;//プレイヤーの火力を装備レベルによって変える(+ポッドレベル)
						...
						m_podhp -= m_Enemy_damage * damage_buff[0] * 1.25f * m_One_pat_dem + (float)g_Def_equip_Level + (float)(g_Bar_Level - 1);
					}
					else if (hit->CheckObjNameHit(OBJ_PODS) != nullptr)		//プレイヤーのグリーンポッド当たり時のHP
					{
						//m_Enemy_damage += (float)g_Spe_equip_Level;//プレイヤーの火力を装備レベルによって変える(+ポッドレベル)
						...
						m_podhp -= m_Enemy_damage * damage_buff[0] * 1.25f * m_One_pat_dem + (float)g_Spe_equip_Level + (float)(g_Bar_Level - 1);
					}
					else if (hit->CheckObjNameHit(OBJ_PODB) != nullptr)	//プレイヤーのホワイトポッド当たり時のHP
					{
						//m_Enemy_damage += (float)g_Bal_equip_Level;//プレイヤーの火力を装備レベルによって変える(+ポッドレベル)
				...
				if (hit->CheckObjNameHit(OBJ_ROCKET) != nullptr)//敵のミサイルに当たった時のHP処理
				{
					...
					//m_Enemy_damage *= m_One_pat_dem;//ワンパターンデメリット値の乗算を行う
						...
						m_podhp -= m_pod_max_hp;//バランスポッドを絶対に破壊する
				}
			}
			...
			hit->CheckElementHit(ELEMENT_ENEMYRKT) == true) && m_type == 0)
		{
			...
			if (ButtonUP == 1)		//自分の種類１(レッド)が敵のポッドと当たった場合
			{
				if (m_atk_cnt == 0)	//0の時のみ、各ダメージを受ける
				{
					if (hit->CheckObjNameHit(OBJ_PODS) != nullptr)			//敵のグリーンポッド当たり時のHP
					{
						//m_Player_damage *= m_One_pat_dem;//ワンパターンデメリット値の乗算を行う

						m_podhp -= m_Player_damage * damage_buff[1] * 0.4f * m_One_pat_dem;
					}
					else if (hit->CheckObjNameHit(OBJ_PODD) != nullptr)		//敵のブルーポッド当たり時のHP
					{
						//m_Player_damage *= m_One_pat_dem;//ワンパターンデメリット値の乗算を行う

						m_podhp -= m_Player_damage * damage_buff[1] * 1.5f * m_One_pat_dem;
					}
			...
			else if (ButtonUP == 2)	//自分の種類２(ブルー)が敵のポッドと当たった場合
			{
				if (m_atk_cnt == 0)	//0の時のみ、各ダメージを受ける
				{
					if (hit->CheckObjNameHit(OBJ_PODP) != nullptr)			//敵のレッドポッド当たり時のHP
					{
						//m_Player_damage *= m_One_pat_dem;//ワンパターンデメリット値の乗算を行う
			...
			else if (ButtonUP == 3)//自分の種類３(グリーン)が敵のポッドと当たった場合
			{
				if (m_atk_cnt == 0)	//0の時のみ、各ダメージを受ける
				{
					if (hit->CheckObjNameHit(OBJ_PODP) != nullptr)			//敵のレッドポッド当たり時のHP
			...
			else if (ButtonUP == 4)//自分の種類４(ホワイト)が敵のポッドとミサイルに当たった場合
			{
				if (m_atk_cnt == 0)	//0の時のみ、ダメージを受ける
				{
					if (hit->CheckObjNameHit(OBJ_PODP) != nullptr)			//敵のレッドポッド当たり時のHP
				...
				if (hit->CheckObjNameHit(OBJ_ROCKET) != nullptr)//敵のミサイルに当たった時のHP処理
				{
					...
					m_podhp -= m_pod_max_hp;//バランスポッドを絶対破壊する
				}
			}
