...
void CObjFight::Action()
{
	...
	if (m_end_f == true)
	{
		...
		{					
			CObjFightClear* crer = new CObjFightClear(5000, 100, L"木材", &g_Wood_num, 10, L"鉄", &g_Iron_num, 40, 2);
			Objs::InsertObj(crer, OBJ_FIGHT_CLEAR, 15);
		}
		//左から２番目
		if (g_Challenge_enemy == 1)									
		{				
			CObjFightClear* crer = new CObjFightClear(10000, 200, L"アルミニウム", &g_Aluminum_num, 60, L"レアメタル", &g_Raremetal_num, 50, 1);
			Objs::InsertObj(crer, OBJ_FIGHT_CLEAR, 15);
		}
		//左から３番目
		if (g_Challenge_enemy == 2)									
		{
			CObjFightClear* crer = new CObjFightClear(15000, 250, L"プラスチック", &g_Plastic_num, 40, L"ガス", &g_gus_num, 45, 5);
			Objs::InsertObj(crer, OBJ_FIGHT_CLEAR, 15);
		}
		//左から４番目
		if (g_Challenge_enemy == 3)									
		{
			CObjFightClear* crer = new CObjFightClear(7000, 150, L"鉄", &g_Iron_num, 60, L"銀", &g_Silver_num, 45, 3);
			Objs::InsertObj(crer, OBJ_FIGHT_CLEAR, 15);
		}
		//ボス
